class BinsController < ApplicationController
  ...
  def bin_params
    @params ||= begin
      request_params = params.require(:bin).permit(:title, :abbreviation, :description, :position, :logo, :post_ids => (0..100).to_a.map(&:to_s), :posts => [:title, :link, :duration, :text_content] )
