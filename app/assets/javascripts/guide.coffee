class @Guide
  constructor: (options) ->
    ...
    $('.guide-container td').on 'click', guideClick
    ...
    window.guideBuildAndSelect = (data) ->
      ...
      $('.selected-show').on 'click', guideClick
      $('.selected-show').scrollintoview()

    window.guideBuild = (data) ->
      if $("#channel-contents tr[data-guide-bin-id='#{data.bin_id}']").length
        $("#guide-contents tr[data-guide-bin-id='#{data.bin_id}']").prepend("<td data-guide-post-id=\"#{data.id}\"><button></button></td>")
        $("#guide-contents tr[data-guide-bin-id='#{data.bin_id}'] td[data-guide-post-id='#{data.id}'] button").text(data.title).attr('title', data.title).on 'click', guideClick
      else
        $("#channel-contents table tbody").append("<tr data-guide-bin-id=\"#{data.bin_id}\"><td class=\"headcol\" data-guide-post-id=\"#{data.id}\" title=\"#{data.bin_title}\"><button>#{data.bin_number} #{data.bin_abbreviation}</button></td></tr>")
        $("#guide-contents table tbody").append("<tr data-guide-bin-id=\"#{data.bin_id}\"><td data-guide-post-id=\"#{data.id}\"><button></button></td></tr>")
        $("#guide-contents tr[data-guide-bin-id='#{data.bin_id}'] td[data-guide-post-id='#{data.id}'] button").text(data.title).attr('title', data.title).on 'click', guideClick
